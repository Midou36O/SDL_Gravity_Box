...
void GameLoop::init() {
  ...
  // OPENGL CODE: MOVE IT TO ITS DESIGNATED FILE!
...
// Declaration of values of the gameloop.
...
void GameLoop::loop() {
  while (gameRunning) {
    ...
    // FPS window
    ImGui::Begin("FPS");
    ImGui::Text("FPS: %f", getframes(nowTicks));
...
float GameLoop::getframes(Uint32 start) {
  Uint32 lastTicks = start;
  // Get the delta time.
  Uint32 nowTicks = SDL_GetTicks64();
